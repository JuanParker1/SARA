    ...
    public function postPgpNt()
    {
    	...
    	$InsertQuery = " 
    		...
    		SELECT ('CLI'||ing.INGCODING) AS IDEV
				...
				,ej.DICODDIAG AS DXPPAL ,ej.DICODDIAG AS DXREL 
				,ej.INGCODING
			...
			FROM BDSALUD.TBEJECAPI ej
				...
				LEFT JOIN BDSALUD.TBSRONCPBL onc ON ( be.BECODBENE = onc.BECODBENE ) -- Oncologicos
			...
			UNION ALL  -- Citas
			SELECT ('CIT'||cit.CICODCITA) AS IDEV
				,INT(cit.CIFECCITA/100) AS PERIODOEV ,cit.CIFECCITA AS FECHAEV ,cit.CIFECCITA AS FECHAING ,cit.CIFECCITA AS FECHASAL
				,be.BECODBENE  ,BDSALUD.FNDOCBENE( be.BECODBENE ) AS PACIENTE ,BDSALUD.FNNOMBBENE( be.BECODBENE ) AS NOMBRE ,be.TDTIPDOC ,be.BENUMDOCBE
				,ac.ACCODACTIV ,ac.ACDESACTIV ,ac.ACCODAGR AS CODCUP ,TRIM(cu.ACDESACTIV) AS CUP ,ac.CTCCLASF1
				,gr.ID AS GRUPOID ,gr.GRUPO ,gr.SUBGRUPO ,gr.PRIORIDAD ,gr.AMBITO
				...
				,CASE WHEN onc.ID IS NULL THEN 'NO' ELSE 'SI' END AS ONCOLOGICO ,ej.GRUPO AS GRUPODX
				,'' AS ESTADOING 
				,BDSALUD.FNDXCITINI( cit.CICODCITA ) AS DXPPAL ,ej.DICODDIAG AS DXREL 
				,ej.INGCODING 
				,ej.TSCODSERV ,BDSALUD.FNDESCSERV( ej.TSCODSERV ) AS SERVICIO
				,ROW_NUMBER() OVER ( PARTITION BY cit.CICODCITA ORDER BY PRIORIDAD ASC, TARIFA DESC ) AS LINEA 
			FROM BDSALUD.TBEJECAPI ej 
			...
			UNION ALL
			SELECT ('ATE'||f.MRCODCONS) AS IDEV
				...
				,ej.INGCODING 
				,ej.TSCODSERV ,BDSALUD.FNDESCSERV( ej.TSCODSERV ) AS SERVICIO
				,ROW_NUMBER() OVER ( PARTITION BY f.MRCODCONS ORDER BY PRIORIDAD ASC, TARIFA DESC ) AS LINEA 
			FROM BDSALUD.TBEJECAPI ej 
