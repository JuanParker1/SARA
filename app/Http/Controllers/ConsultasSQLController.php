...
class ConsultasSQLController extends Controller
{
    ...
    public function postPgpNt()
    {
		...
		$InsertQuery = " 
			...
			UNION ALL
			SELECT  
				('ATE'||f.MRCODCONS||'_'||ac.ACCODAGR) AS IDEV
				,INT(F.MRFECATE/100) AS PERIODOEV
				,f.MRFECATE AS FECHAEV
				,f.MRFECATE AS FECHAING
				,f.MRFECATE AS FECHASAL
				,be.BECODBENE  ,BDSALUD.FNDOCBENE( be.BECODBENE ) AS PACIENTE ,BDSALUD.FNNOMBBENE( be.BECODBENE ) AS NOMBRE ,be.TDTIPDOC ,be.BENUMDOCBE
				,ac.ACCODACTIV ,ac.ACDESACTIV ,ac.ACCODAGR AS CODCUP ,TRIM(cu.ACDESACTIV) AS CUP ,ac.CTCCLASF1
				,gr.ID AS GRUPOID ,gr.GRUPO ,gr.SUBGRUPO ,gr.PRIORIDAD ,gr.AMBITO
				...
				,'' AS ESTADOING
				,ej.DICODDIAG AS DXPPAL ,'' AS DXREL
				,ej.INGCODING ,CIT.CICODCITA ,ej.MRCODCONS ,ej.MRCODCONS AS CODCONT
				,ej.TSCODSERV ,BDSALUD.FNDESCSERV( ej.TSCODSERV ) AS SERVICIO
			...
			FROM BDSALUD.TBEJECAPI ej
				...
				LEFT JOIN BDSALUD.TBAGCITAS CIT ON (EJ.MRCODCONS=CIT.MRCODCONS)
				...
				LEFT JOIN BDSALUD.TBFACUPS   cu  ON ( ac.ACCODAGR = cu.ACCODACTIV ) -- Codigos CUPS
			WHERE 1 = 1
				AND ej.INGCODING IS NULL AND ej.CICODCITA IS NULL AND ej.MRCODCONS IS NOT NULL AND CIT.CICODCITA IS NULL
				AND f.MRFECATE = $Dia 
				AND (ac.CTCCLASF1 NOT IN ('NPO') OR ac.CTCCLASF1 IS NULL) -- Solicitado Alexis 15 Mar 2022
			UNION ALL
			SELECT  
				('ATE'||f.MRCODCONS||'_'||ac.ACCODAGR) AS IDEV
				,INT(CIT.CIFECCITA/100) AS PERIODOEV
				,CIT.CIFECCITA AS FECHAEV
				,CIT.CIFECCITA AS FECHAING
				,CIT.CIFECCITA AS FECHASAL
				,be.BECODBENE  ,BDSALUD.FNDOCBENE( be.BECODBENE ) AS PACIENTE ,BDSALUD.FNNOMBBENE( be.BECODBENE ) AS NOMBRE ,be.TDTIPDOC ,be.BENUMDOCBE
				,ac.ACCODACTIV ,ac.ACDESACTIV ,ac.ACCODAGR AS CODCUP ,TRIM(cu.ACDESACTIV) AS CUP ,ac.CTCCLASF1
				,gr.ID AS GRUPOID ,gr.GRUPO ,gr.SUBGRUPO ,gr.PRIORIDAD ,gr.AMBITO
				,COALESCE(ej.ENCODENTID, f.ENCODENTID) AS ENCODENTID ,COALESCE(ej.EPCODPLAN, f.EPCODPLAN) AS EPCODPLAN
			...
			FROM BDSALUD.TBEJECAPI ej
				...
				LEFT JOIN BDSALUD.TBFACUPS   cu  ON ( ac.ACCODAGR = cu.ACCODACTIV ) -- Codigos CUPS
			WHERE 1 = 1
				AND ej.INGCODING IS NULL AND ej.CICODCITA IS NULL AND ej.MRCODCONS IS NOT NULL AND CIT.CICODCITA IS NOT NULL
				AND CIT.MOCODMOTIV=06
				AND CIT.CIFECCITA = $Dia 
				AND (ac.CTCCLASF1 NOT IN ('NPO') OR ac.CTCCLASF1 IS NULL) -- Solicitado Alexis 15 Mar 2022
			UNION ALL 
		...
		$InsertQuery2 = "INSERT INTO ZZVISTASAL.VMPGPEV  
		    	SELECT det.IDEV
					...
					,CASE WHEN det.ESTADOING NOT LIKE '' THEN 1 ELSE 
					 ( SELECT SUM(d2.CANT)   FROM ZZVISTASAL.VMPGPEVDET d2 WHERE d2.IDEV = det.IDEV AND det.FECHAEV = d2.FECHAEV ) END AS CANT
