...
{    
    ...
    protected $variables = [];
    protected $valid_variables = ['a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z'];
    ...
    public function setFormula($formula = "", $precision = 4)
    {
        if (!empty($formula))
            $this->formula = $this->original_formula = trim($formula);
    ...
    public function getResult()
    {
        ...
        for ($i=0; $i<=count($this->valid_variables)-1; $i++) { 
            ...
            //|| $this->valid_variables[$i] == "e" --Ajuste quitar euler
            ) {
                $this->correct = false;
                $this->error_type = 4;
            }
