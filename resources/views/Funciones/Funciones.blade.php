<div flex id="Funciones" layout ng-controller="FuncionesCtrl">
	
	<md-sidenav class="bg-white border-radius border margin-5 w350" layout=column 
		md-is-open="FuncionesNav"
		md-is-locked-open="$mdMedia('gt-xs') && FuncionesNav">
		...
		<div layout class="border-bottom h30" layout-align="center center">
			<div class="md-toolbar-searchbar" flex layout>
				...
				<input flex type="search" placeholder="Buscar Función..." ng-model="filterFunciones" class="no-padding" ng-change="searchFuncion()" ng-model-options="{ debounce : 500 }">
			</div>
			<md-button class="md-icon-button no-margin" aria-label="b" ng-click="addFuncion()">
				<md-icon md-font-icon="fa-plus"></md-icon>
				<md-tooltip md-direction=left>Agregar Función</md-tooltip>
			</md-button>
		</div>

		<div layout=column flex class="overflow-y darkScroll padding-top-5">

			<div ng-repeat="F in FuncionesFS" class="mh25 borders-bottom padding-0-5 relative text-13px show-child-on-hover"
				md-ink-ripple layout ng-show="F.show">
				<div ng-style="{ width: (F.depth * 12) }"></div>
				<div ng-show="F.type == 'folder'" flex layout ng-click="FsOpenFolder(FuncionesFS, F)" class="Pointer">
				...
				<div ng-show="F.type == 'file'" flex layout class="Pointer" ng-click="openFuncion(F.file)" 
	...
	<div flex class="" layout=column ng-show="FuncionSel">
		<div flex layout=column class="overflow-y darkScroll padding-5">
			<div layout class="">
				<md-button class="md-icon-button no-margin no-padding s30" aria-label="b" ng-click="FuncionesNav = !FuncionesNav" 
					style="margin-top: 2px !important">
					<md-icon md-svg-icon="md-bars" class=""></md-icon>
				</md-button>
				<md-input-container class="no-margin-top no-margin-bottom" flex>
					<input type="text" ng-model="FuncionSel.Titulo" aria-label=s>
				</md-input-container>
			</div>

			<md-chips class="compact" ng-model="FuncionSel.Secciones" placeholder="Secciones" md-enable-chip-edit></md-chips>
							...
							<tr md-row class="" ng-repeat="C in CardsCRUD.rows | orderBy:'Indice'" ng-class="{'bg-yellow': C.changed}">
								<td md-cell class="md-cell-compress">
									<md-select class="w100p" ng-model="C.seccion_id" aria-label=s ng-change="C.changed = true">
									  ...
									  <md-option ng-value="k" ng-repeat="(k,S) in FuncionSel.Secciones">{{ S }}</md-option>
									</md-select>
								</td>
								<td md-cell class="md-cell-compress">
		...
		<div layout class="border-top bg-lightgrey-5">
			<md-menu>
				...
				<md-menu-content>
					<md-menu-item><md-button ng-click="copyFuncion()"><md-icon md-font-icon="fa-copy margin-right fa-fw"></md-icon>Copiar Funcion</md-button></md-menu-item>
				</md-menu-content>
			</md-menu>
			<md-button class="md-icon-button no-margin" aria-label="b" ng-click="viewFuncionDiag(FuncionSel.id)">
				<md-icon md-font-icon="fa-external-link-alt fa-fw fa-lg"></md-icon>
			</md-button>
			<span flex></span>
			<md-button class="md-primary md-raised mh30 h30 lh30" ng-click="updateFuncion()">
